...
ThreatAssess = function(full_name,ID_list,path,LC.points = FALSE,SIS.files = FALSE, BigKeys) {
  ...
  if (nrow(good.species) >= 1) {
    ...
    if (nrow(big_keys) >= 1){
      ...
      print(paste0("Warning - these species contain >200,000 points and are too big to download from GBIF through this method. See outputs folder for a list of GBIF codes and use the bigkeys function to download them."))
      print(paste0("GBIF key:",big_keys))
            }
  ...
  ######### now add the model and predict threat status
  ...
  fam <- read.csv("fam_converted.csv")
  ...
  ######## - Print that results have been written
