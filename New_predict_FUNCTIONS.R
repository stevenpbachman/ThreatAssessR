...
### RedLeast - or new name 'threatAssessR' gather parameters for plant species using GBIF and POWO (distribution, EOO, AOO etc.)
...
LC.biorealms = function(TDWG_realms, result.tdwg) {
  ...
  biorealm_summary = realms %>% dplyr::group_by(POWO_ID) %>% dplyr::summarise(newREALM = paste(unique(REALM), collapse =
                                                                                   ","))
...
LC.variables = function(result.tdwg.native, unique.sp) {
  ...
  extras = result.tdwg.native %>% dplyr::group_by(POWO_ID) %>% dplyr::summarise(
...
RedLeast = function(full_name,ID_list,path,LC.points = FALSE,SIS.files = FALSE, BigKeys) {
  ...
  if (nrow(good.species) >= 1) {
    ...
    if (nrow(withPoints.species) >= 1) {
      ...
      if (nrow(PointsInRange.species) >= 1) {
        ...
        if (LC.points) {
          all_points = lapply(unique.sp, LC.point.file, native_only, result.table)
        ...
        if (SIS.files) {
          ...
          #sis.credits = LC.credits(result.table)
          #sis.taxonomy = LC.taxonomy(result.table, IUCN_taxonomy)
          #sis.assessments = LC.assessments(result.table, biorealms)
          #sis.countries = lapply(unique.sp, LC.countries, result.tdwg)
          #sis.countries = do.call(rbind, sis.countries)
          #sis.allfields = LC.allfields(result.table)
          #sis.plantspecific = LC.plantspecific(result.table)
          #sis.habitats = LC.habitats(result.table)
          #sis.references = LC.references(result.table)
          LC.SIS.all(result.table,unique.sp,result.tdwg,IUCN_taxonomy,biorealms)
  ...
  if (!exists("result.table.errors")) {
    ...
    result.table = result.table.errors
  } else {
    #result.table = LC.results(result.table)
  ...
  ####### Need to filter on those in the results where there is missing data
  ####### add 'threat' column 
  ####### update those with missing data as Insufficient Data for prediction - not DD, as this is IUCN Category
  
  # apply the prediction model - function all this up?
  # load first
  RedLeastRFmodel = readRDS("C:/Users/sb42kg/OneDrive - The Royal Botanic Gardens, Kew/02_Publications/spb/Machine learning - big data red list/05_R/03_Model_Predict/RedLeastRFmodel.rds")
  ...
  #save.results = write.table( result.predict,respath,row.names = FALSE,na = "",sep = ",")
  
  # change family to factors so that it can be used in the model (as integers)
  # load the input data where you have original family names and integers
  fam <- read_csv("C:/Users/sb42kg/OneDrive - The Royal Botanic Gardens, Kew/02_Publications/spb/Machine learning - big data red list/05_R/03_Model_Predict/fam_converted.csv")
  
  # map this to the results table - merge
  result.predict = merge(fam, result.predict, by = "fam", all.y = TRUE)
  ...
  result.predict$EOO = as.numeric(result.predict$EOO)
  result.predict$AOO = as.integer(result.predict$AOO)
  result.predict$norm_forest_loss = as.numeric(result.predict$norm_forest_loss)
  result.predict$norm_hfoot = as.integer(result.predict$norm_hfoot)
  
  #str(result.predict)
  
  # there may still be some NAs here - so model won't run
  # take these out and update result.table with DD flag
  na_rows = result.predict[rowSums(is.na(result.predict))>0,]
  na_rows$threat_prediction = "Insufficient Data"

  # these are the good ones for the model
  result.predict = anti_join(result.predict,na_rows, "POWO_ID")
  
  # use model to predict data from result.table
  threat_prediction <- predict(object = RedLeastRFmodel,  
                               newdata = result.predict,  
                               type = "raw")   
  
  threat_prediction = as.data.frame(threat_prediction)
  
  # join back to result table
  model_result.table = cbind(result.predict,threat_prediction)
  #save.results = write.table(   result.table,respath,row.names = FALSE,na = "",sep = ",")
...
####### add the if statement to trigger this
# for (i in (result.table$full_name)){
#      rmarkdown::render(input = 'C:/Users/sb42kg/OneDrive - The Royal Botanic Gardens, Kew/02_Publications/spb/Machine learning - big data red list/05_R/04_Output_Assessments/markdown_species_reports/r_markdown.Rmd',
#                      output_file = paste("report_",i,".html", sep=""),
#                      output_dir =  'C:/Users/sb42kg/OneDrive - The Royal Botanic Gardens, Kew/02_Publications/spb/Machine learning - big data red list/05_R/04_Output_Assessments/markdown_species_reports/')
# }
#  
# Try cross talk https://www.rstudio.com/resources/videos/linking-html-widgets-with-crosstalk/
